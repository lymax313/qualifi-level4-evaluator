...
Evaluate this assignment and provide scores for each criterion.
Return ONLY valid JSON in this format:
{{
    "learning_outcomes": [
        {{"outcome": "LO description", "achieved": "Y", "evidence": "evidence text"}}
    ],
    "rubric_scores": {{
        "Content": {{"score": 35, "feedback": "feedback text"}},
        "Application of Theory and Literature": {{"score": 30, "feedback": "feedback text"}},
        "Knowledge and Understanding": {{"score": 40, "feedback": "feedback text"}},
        "Presentation/Writing Skills": {{"score": 30, "feedback": "feedback text"}},
        "Referencing": {{"score": 30, "feedback": "feedback text"}}
    }},
    "overall_feedback": "feedback text",
    "strengths": ["strength1"],
...
def generate_pdf(student_name: str, student_id: str, unit_title: str, evaluation: dict) -> str:
    ...
    for lo in evaluation.get("learning_outcomes", []):
        ...
        outcome_text = lo.get('outcome', 'N/A')[:80]
        achieved_status = "[PASS] ACHIEVED" if lo.get("achieved") == "Y" else "[FAIL] NOT ACHIEVED"
        pdf.cell(0, 6, f"{achieved_status}: {outcome_text}", 0, 1)
        
        pdf.set_font("Arial", "", 9)
        evidence_text = lo.get('evidence', 'N/A')[:150]
    ...
    for criterion, max_score in QUALIFI_RUBRIC.items():
        if criterion in evaluation.get("rubric_scores", {}):
            ...
            feedback = data.get("feedback", "N/A")[:200]
    ...
    overall_feedback = evaluation.get("overall_feedback", "N/A")
    pdf.multi_cell(0, 6, overall_feedback)
